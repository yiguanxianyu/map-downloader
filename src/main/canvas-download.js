...
const destCoords2Window = (destCoords, boundingBox, unitLngPerPixel, unitLatPerPixel) => {
  ...
  const startY = Math.floor((destCoords[1] - boundingBox.latMax) / unitLatPerPixel)
...
class datasetHandler {
  ...
  async download(outputPath) {
    ...
    const allPromises = []
    console.time('Download time cost')
    ...
    console.timeEnd('Download time cost')
    ...
    const metadata = {
      ...
      ModelTiepoint: [0, 0, 0, this.destCoords[0], this.destCoords[1], 0],
      SamplesPerPixel: 3,
    ...
    // const pngData = await this.canvas.encode('png') // JPEG, AVIF and WebP are also supported
    // // encoding in libuv thread pool, non-blocking
    // await promises.writeFile('simple.png', pngData)

    // await sharp(imageDataWithoutAlpha,
